...
public class ExeApp {
	...
	public ExeApp() {
		...
		while (true) {
			...
			// Tip! ctrl 누른채로 showMenu()에 마우스 갖다대봐..!
			
			// 1) 메뉴 출력
			showMenu(); // while문이 돌 때마다 메뉴를 출력해줌
			System.out.println("메뉴 선택 > ");
			
			// 2) 선택할 메뉴 입력받기
			menuNo = Integer.parseInt(sc.nextLine());
			
			// 3) switch문을 사용해 입력한 메뉴로 갈 수 있도록 하기
			switch (menuNo) {
			case 1:
				setSize();
				break;
	...
	public void setSize() {
		...
		Bary = new Bank[count];  // 객체 배열☆☆☆☆☆☆☆☆☆☆☆☆☆☆☆☆☆☆☆☆☆☆☆☆☆☆☆☆☆☆☆
		// 나중에 배우는 List가 객체 배열의 상위 버전임..!
	}
	// 이제 setSize를 부르면 이 메소드가 작동할 것!
